                ...
                <div class="container">
                    <div class="card mx-auto p-5">
                        <div class="mb-3 flex justify-between items-center">
                            <h1 class="text-xl font-medium">Detail post</h1>
                        </div>
                        <div class="mt-5" >
                            ...
                            <div>
                                <div class="mb-6">
                                    ...
                                    <input type="text" disabled id="title" name="title" value="${post.postTitle}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"  >
                                </div>
                                <div class="mb-6">
                                    <div>
                                    ...
                                    <select disabled id="category" name="category"
                                        ...
                                        <c:forEach items="${categories}" var="item">
                                            <option value="${item.settingID}" ${post.postCategory.settingID==item.settingID ? "selected": ""}>${item.name}</option>
                                        </c:forEach>
                                    </select>
                                </div>
                                <div class="flex items-center mb-4">
                                    <c:forEach items="${status}" var="item">
                                        <div class="mr-3 flex items-center">
                                            <input name="status" disabled id="status-${item.settingID}" ${post.postStatus.settingID==item.settingID ? "checked": ""} type="radio" value="${item.settingID}" class="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300 focus:ring-blue-500 focus:ring-2">
                                        ...
                                        <label for="thumbnail" class="flex flex-col justify-center items-center w-full h-64 bg-gray-50 rounded-lg border-2 border-gray-300 border-dashed cursor-pointer hover:bg-gray-100">
                                            <div class="flex flex-col justify-center items-center pt-5 pb-6">
                                                <div class="max-w-[400px] max-h-[240px]">
                                                    <img id="image-preview" class="w-full h-full" src="${pageContext.request.contextPath}/View/imgpost/${post.thumbnail}"/>
                                                </div>
                                            ...
                                            <input id="thumbnail" disabled name="thumbnail" type="file" accept="image/*" class="hidden" />
                                        </label>
                                    </div> 
                                </div>
                                <div class="mb-6 mt-6">
                                    <label for="summary" class="block mb-2 text-sm font-medium text-gray-900">Description</label>
                                    <textarea id="description" disabled name="summary" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500" placeholder="Description...">${post.summary}</textarea>
                                </div>
                                <div>
                            ...
                            <a href="${pageContext.request.contextPath}/posteditcontroller?id=${post.postId}" class="block mt-5 ml-auto text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">Edit</a>
                        </div>
                    </div>
                </div>
