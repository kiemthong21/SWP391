<%-- 
    ...
    Created on : Jul 20, 2022, 10:27:32 PM
    Author     : Hieuhihi
--%>
                        ...
                        <form class="mt-5" method="POST" enctype="multipart/form-data" id="form-post">
                            ...
                            <div>
                                <div class="mb-6">
                                    ...
                                    <input type="text" id="title" name="title" value="${product.title}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"  >
                                </div>
                                <div class="mb-6">
                                    <div>
                                    ...
                                    <select id="category" name="category"
                                        ...
                                        <c:forEach items="${categories}" var="item">
                                            <option value="${item.settingID}" ${product.category.settingID==item.settingID ? "selected": ""}>${item.name}</option>
                                        </c:forEach>
                                    </select>
                                </div>
                                <div>
                                ...
                                <div class="flex items-center mb-4">
                                    <c:forEach items="${status}" var="item">
                                        <div class="mr-3 flex items-center">
                                            <input name="status" id="status-${item.settingID}" ${product.status.settingID==item.settingID ? "checked": ""} type="radio" value="${item.settingID}" class="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300 focus:ring-blue-500 focus:ring-2">
                                        ...
                                        <label for="thumbnail" class="flex flex-col justify-center items-center w-full h-64 bg-gray-50 rounded-lg border-2 border-gray-300 border-dashed cursor-pointer hover:bg-gray-100">
                                            <div class="flex flex-col justify-center items-center pt-5 pb-6">
                                                <div class="max-w-[400px] max-h-[240px]">
                                                    <img id="image-preview" class="w-full h-full" src="${pageContext.request.contextPath}/View/Img/${product.thumbnail}"/>
                                                </div>
                                                <div id="icon-upload" class="hidden flex items-center flex-col justify-center">
                                ...
                                <div class="mb-6">
                                    ...
                                    <input type="text" id="summary" name="summary" value="${product.summary}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"  >
                                </div>
                                <div class="mb-6">
                                    <label for="title" class="block mb-2 text-sm font-medium text-gray-900">Price</label>
                                    <input type="text" id="price" name="price" value="${product.price}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"  >
                                </div>
                                <div class="mb-6">
                                    <label for="title" class="block mb-2 text-sm font-medium text-gray-900">Discount</label>
                                    <input type="text" id="discount" name="discount" value="${product.discount}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"  >
                                </div>
                                <div class="mb-6">
                                    <label for="title" class="block mb-2 text-sm font-medium text-gray-900">Quantity</label>
                                    <input type="text" id="quantity" name="quantity" value="${product.quantity}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"  >
                                </div>
                            </div>
                            <button type="submit" class="mt-5 ml-auto text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">Submit</button>
                        </form>
                    </div>
                </div>
